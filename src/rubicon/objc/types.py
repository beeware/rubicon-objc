...
def ctype_for_encoding(encoding):
    ...
    ``ctype`` is then created from the converted ctypes, and is registered for
...
def encoding_for_ctype(ctype):
    ...
    returned. Otherwise, if the ``ctype`` is a pointer type, its pointed-to type
...
def unregister_ctype(ctype):
    """Unregister the conversion from a ``ctype`` to its corresponding
...
def unregister_ctype_all(ctype):
    ...
    All conversions from any type encoding to this ``ctype`` are removed
...
def get_ctype_for_encoding_map():
    """Get a copy of all currently registered encoding-to-``ctype`` conversions
    as a map."""
...
def get_encoding_for_ctype_map():
    """Get a copy of all currently registered ``ctype``-to-encoding conversions
    as a map."""
